...
- 当用户 **@torch_npu_doc** 或明确要求查阅 torch_npu 中文文档时：
  - **优先使用本 skill 的 reference 文档**：[reference.md](references/reference.md)。其中包含项目内全部中文文档的索引与关键内容摘要。
  - 根据用户问题在 reference.md 的「文档索引」中定位到对应文档路径（如 `skills/base/torch_npu/README.zh.md`、`skills/base/torch_npu/docs/zh/quick_start/quick_start.md`、`skills/base/torch_npu/docs/zh/framework_feature_guide_pytorch/`、`skills/base/torch_npu/docs/zh/troubleshooting/` 等），按需读取工作区内该文件并引用相关段落。
...
- **本 skill 文档参考**：[reference.md](references/reference.md)。汇总 torch_npu 项目内所有中文文档索引与要点；用户 **@torch_npu_doc** 时据此定位并引用具体文档。
- **项目中文说明**：工作区内 `skills/base/torch_npu/README.zh.md`（版本表、部署、验证、参考文档链接）。
