...
| 路径 | 说明 |
|------|------|
| `base/torch_npu/README.zh.md` | 项目总览、PyTorch/Python 版本配套表、昇腾辅助软件表、环境部署（二进制/源码）、安装后验证、卸载、硬件配套、分支维护策略、参考文档链接 |
| `base/torch_npu/CONTRIBUTING.zh.md` | 贡献指南：CLA、Fork、测试用例、代码风格、门禁、Fork-Pull、报告问题、提出 PR |

### 2. docs/zh 概览与入门

| 路径 | 说明 |
|------|------|
| `base/torch_npu/docs/zh/overview/product_overview.md` | Ascend Extension for PyTorch 是什么、总体架构、关键功能特性 |
| `base/torch_npu/docs/zh/quick_start/quick_start.md` | 快速入门：环境准备、模型迁移训练（GPU→NPU 示例）、混合精度说明 |
| `base/torch_npu/docs/zh/release_notes/release_notes.md` | 版本说明、PyTorch/CANN/Extension 配套表、组件配套、维护策略 |
| `base/torch_npu/docs/zh/security_statement/security_statement.md` | 安全声明 |

### 3. docs/zh 框架特性指南（framework_feature_guide_pytorch）

路径前缀：`base/torch_npu/docs/zh/framework_feature_guide_pytorch/`
...
路径前缀：`base/torch_npu/docs/zh/troubleshooting/`
...
2. **按需打开工作区内对应文件**（如 `base/torch_npu/README.zh.md`、`base/torch_npu/docs/zh/quick_start/quick_start.md`、`base/torch_npu/docs/zh/framework_feature_guide_pytorch/xxx.md`、`base/torch_npu/docs/zh/troubleshooting/xxx.md`），提取与问题相关的段落或步骤。
